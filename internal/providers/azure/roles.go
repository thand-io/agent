...
func (p *azureProvider) getRoleDefinition(ctx context.Context, roleName string) (*armauthorization.RoleDefinition, error) {
	...
	// Sanitise the role name for OData filter (escape embedded single quotes)
	safeRoleName := strings.ReplaceAll(roleName, "'", "''")
	roleFilter := fmt.Sprintf("roleName eq '%s'", safeRoleName)
...
func (p *azureProvider) createRoleDefinition(ctx context.Context, roleName, description string, permissions models.RolePermissions) (*armauthorization.RoleDefinition, error) {
	...
	// Convert permissions to Azure actions, notActions, dataActions, and notDataActions
	actions, notActions, dataActions, notDataActions, targets := permissionsToAzureActions(permissions)

	// Convert to Azure pointer format
	var actionPtrs []*string
	for i := range actions {
		actionPtrs = append(actionPtrs, &actions[i])
	}

	var notActionPtrs []*string
	for i := range notActions {
		notActionPtrs = append(notActionPtrs, &notActions[i])
	}

	var dataActionPtrs []*string
	for i := range dataActions {
		dataActionPtrs = append(dataActionPtrs, &dataActions[i])
	}

	var notDataActionPtrs []*string
	for i := range notDataActions {
		notDataActionPtrs = append(notDataActionPtrs, &notDataActions[i])
	}
	...
	// Note: the vendored armauthorization SDK version represents data-plane
	// permissions (DataActions / NotDataActions) in PermissionAutoGenerated
	// (used by GET), not in Permission (used by CreateOrUpdate). Log a warning
	// when data-plane operations were requested so they are not silently dropped.
	if len(dataActionPtrs) > 0 || len(notDataActionPtrs) > 0 {
		logrus.WithFields(logrus.Fields{
			"role_name":        roleName,
			"data_actions":     len(dataActionPtrs),
			"not_data_actions": len(notDataActionPtrs),
		}).Warn("Azure role contains data-plane operations; upgrade the armauthorization SDK to include DataActions in the role definition")
	}
...
func (p *azureProvider) createRoleAssignment(ctx context.Context, user *models.User, roleDefinitionID string) (string, error) {
	...
	if err != nil {
		// 409 Conflict: the assignment already exists (e.g. Temporal retry after an
		// unacknowledged success). Treat this as idempotent success.
		...
		if errors.As(err, &respErr) && respErr.ErrorCode == "RoleAssignmentExists" {
			logrus.WithFields(logrus.Fields{
				...
				"scope":        scope,
			}).Info("Azure role assignment already exists — treating as success")
...
func (p *azureProvider) deleteRoleAssignment(
	...
	// Sanitise the principal ID for OData filter
	safePrincipalID := strings.ReplaceAll(storedPrincipalID, "'", "''")
	assignFilter := fmt.Sprintf("principalId eq '%s'", safePrincipalID)
...
func (p *azureProvider) getUserPrincipalID(ctx context.Context, user *models.User) (string, error) {
	...
	// Sanitise the email for OData filter to prevent filter injection.
	logrus.WithField("email", user.Email).Debug("Looking up Azure AD object ID via Microsoft Graph API")

	safeEmail := strings.ReplaceAll(user.Email, "'", "''")
	filter := fmt.Sprintf("mail eq '%s' or userPrincipalName eq '%s'", safeEmail, safeEmail)
	...
	userList, err := p.graphClient.Users().Get(ctx, requestConfig)
	if err != nil {
