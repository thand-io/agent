...
func (r *ResumableWorkflowRunner) executeTryTask(
	...
	if workflowTask.GetInstanceCtx() == nil {
		workflowTask.SetInstanceCtx(make(map[string]any))
	}

	contextMap, ok := workflowTask.GetInstanceCtx().(map[string]any)
	if !ok {
		contextMap = make(map[string]any)
		workflowTask.SetInstanceCtx(contextMap)
	}
